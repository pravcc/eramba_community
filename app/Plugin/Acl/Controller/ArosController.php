...
class ArosController extends AclAppController
{
	...
	/**
	 * Set up role-based ACL permissions.
	 * 	
	 * @param  int|null $role_id Role ID to manage permissions for, to optimize performance. Null to manage all groups.
	 */
	function admin_ajax_role_permissions($selected_role_id = null)
	{
		$role_model_name = Configure :: read('acl.aro.role.model');
	    ...
	    $this->{$role_model_name}->recursive = -1;
	    $rolesList = $this->{$role_model_name}->find('list', array(
	    	'fields' => [
	    		$role_model_name . '.' . $this->{$role_model_name}->primaryKey,
	    		$role_model_name . '.' . $role_display_field
	    	],
	    	'order' => $role_display_field,
	    	'recursive' => -1
	    ));

	    // manage permissions is possible only for a selected single role
	    if ($selected_role_id !== null) {
	    	$conds = [
	    		$role_model_name . '.' . $this->{$role_model_name}->primaryKey => $selected_role_id
	    	];

	    	 // this is the list to manage permissions - keeping old style find('all')
	    ...
	    $this->set('selected_role_id', $selected_role_id);
	...
	function admin_grant_all_controllers($role_id)
	{
        ...
        else
        {
            ...
            $permissions = $this->Acl->adapter()->Permission->find('list', [
	        	'conditions' => [
	        		'Permission.aro_id' => $node[0]['Aro']['id']
	        	],
	        	'fields' => [
	        		'id', 'aco_id'
	        	],
	        ...
	        foreach ($permissions as $permissionId => $acoId) {
	        	$acoNode = $this->Acl->Aco->getPath($acoId);

	        	// if is a descendant of controllers node and
	        	// is not a controllers node itself
	        	if ($acoNode[0]['Aco']['id'] == 1 && count($acoNode) > 1) {
	        		$alias = $this->_createAlias($acoNode);

	        		$inherit = $this->Acl->inherit($role, $alias);
	        	}
	        }
	...
	protected function _createAlias($node)
	{
		$alias = [];
		foreach ($node as $n) {
			// this is a bug on corrupted databases
			if ($n['Aco']['alias'] == 'save') {
				continue;
			}
