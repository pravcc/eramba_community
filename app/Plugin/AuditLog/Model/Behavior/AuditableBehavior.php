 ...
 */
	public function afterSave(Model $Model, $created, $options = array()) {
		...
		$Model->Audit = ClassRegistry::init(array('class' => 'ObjectVersion.Audit', 'alias' => 'Audit'));
		...
		//specific functionality for Restore event in Eramba
		if (!empty($Model->objectRestored)) {
			$event = Audit::EVENT_RESTORE;
			$restore_id = $Model->objectRestored;
		}
		else {
		...
		if ($created || count($updates)) {
			...
			$Model->objectVersionHasChanges = true;
		}
		else {
			// temporary hotfix - let the model know that there were no changes done on the save
			$Model->objectVersionHasChanges = false;
		}
 ...
 */
	public function afterDelete(Model $Model) {
		...
		$this->Audit = ClassRegistry::init(array('class' => 'ObjectVersion.Audit', 'alias' => 'Audit'));
		$this->Audit->create();
 ...
 */
	...
	/**
	 * Get list of ignored fields by audit.
	 *
	 * @param Model $Model
	 * @return array List of ignored fields.
	 */
	public function getAuditIgnoredFields(Model $Model)
	{
		$fields = ['workflow_status', 'workflow_owner_id'];

		if (!empty($this->settings[$Model->alias]['ignore'])) {
			$fields = am($fields, ((array) $this->settings[$Model->alias]['ignore']));
		}
