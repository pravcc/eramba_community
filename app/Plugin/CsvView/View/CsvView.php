 ...
 **/
	protected function _setupViewVars() {
		...
		if ($this->viewVars['_newline'] === null) {
			// ERAMBA CODE ADJUSTMENT FROM '\n' to ' ' (space character) for new lines in export
			...
			foreach ($this->viewVars[$viewVar] as $_data) {
				if ($extract === null) {
					...
					foreach ($extract as $e) {
						...
						if (!empty($value)) {
							$values[] = sprintf($format, implode(', ', $value));
						} else {
 ...
 **/
	protected function _generateRow($row = null) {
		...
		//
		// Escape chars "=", "+", "-", "@", to protect CSV file against CSV injection
		if (is_array($row)) {
			$escape_chars = array('=', '+', '-', '@');
			foreach ($row as $key => $col) {
				$col_index = 0;
				while ($col_index < strlen($col)) {
					if (!in_array($col[$col_index], $escape_chars)) {
						break;
					}
		...
		$csv = str_replace($breaks, $newline, $csv);
