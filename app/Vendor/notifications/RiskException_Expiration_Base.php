<?php
class RiskException_Expiration_Base extends NotificationsBase {
	...
	public function __construct($options = array()) {
		...
		if ($days < 0) {
			$this->title = __('Risk Exception Deadline (-%s)', $daysLabel);
			$this->description = __('Notifies %s before a Risk Exception expires', $daysLabel);
			...
			$this->title = __('Risk Exception Deadline (+%s)', $daysLabel);
			$this->description = __('Notifies %s after a Risk Exception expires', $daysLabel);
		...
		$this->conditions[$this->model . '.status !='] = RISK_EXCEPTION_CLOSED;
	}
}
