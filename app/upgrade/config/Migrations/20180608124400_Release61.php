...
class Release61 extends AbstractMigration
{
    protected function bumpVersion($value) {
        ...
        if (class_exists('App')) {
            ...
            if (Configure::read('Eramba.version') === 'e1.0.6.060') {
                // resave dashboard titles (via structure sync)
        ...
        if (!$ret) {
            ...
            $log = "Error occured when processing database synchronization for release 1.0.6.061.";
    ...
    public function up()
    {
        $this->bumpVersion('e1.0.1.041');
    }

    public function down()
    {
        $this->bumpVersion('e1.0.1.040');
    }
}
