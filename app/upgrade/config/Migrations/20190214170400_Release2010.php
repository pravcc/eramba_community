...
class Release2010 extends AbstractMigration
{
    protected function bumpVersion($value) {
        ...
        if (class_exists('App')) {
            ...
            if (Configure::read('Eramba.version') === 'e2.0.9') {
                App::uses('ReportsModule', 'Reports.Lib');
                $ReportsModule = new ReportsModule();
                $ret &= $ReportsModule->updateReports(['SecurityServiceItemDefaultReport', 'SecurityPolicyItemDefaultReport']);

                $AdvancedFilter = ClassRegistry::init('AdvancedFilters.AdvancedFilter');
                $ret &= $AdvancedFilter->syncDefaultIndex(null, ['UserSystemLog']);
            }
