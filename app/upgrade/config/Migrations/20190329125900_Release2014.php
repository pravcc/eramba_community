...
class Release2014 extends AbstractMigration
{
    protected function bumpVersion($value) {
        ...
        if (class_exists('App')) {
            ...
            if (Configure::read('Eramba.version') === 'e2.0.13') {
    ...
    // set filter limits to unlimited
    protected function _updateFilterLimits()
    {
        App::uses('AdvancedFilterValue', 'AdvancedFilters.Model');
        
        return (bool) ClassRegistry::init('AdvancedFilters.AdvancedFilterValue')->updateAll([
            'AdvancedFilterValue.value' => '-1'
        ], [
            'AdvancedFilterValue.field' => '_limit'
        ]);
    }

    public function up()
    {
        $this->bumpVersion('e2.0.14');
    }

    public function down()
    {
        $this->bumpVersion('e2.0.13');
    }
}
